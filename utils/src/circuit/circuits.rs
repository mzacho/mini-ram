...
pub fn select_const(c1: u32, c2: u32) -> Circuit<u32> {
    let n_in = 1;
    let x = ARG0;
    let mut b = Builder::new(n_in);
...
/// A circuit that computes select_const(i, cs)
pub fn select_const_vec(cs: &[u32]) -> Circuit<u32> {
    let n_in = 1;
    let x = ARG0;
    let mut b = Builder::new(n_in);
    let mut ids = vec![];
...
pub fn encode4(c: u32) -> Circuit<u32> {
    let n_in = 4;
    let x0 = ARG0;
    let mut b = Builder::new(n_in);
    let y = b.encode4(x0);
...
pub fn check_all_eq_but_one() -> Circuit<u32> {
    let n_in = 5;
    let x = ARG0;
    let mut b = Builder::new(n_in);
    let xs = &[(ARG0 + 1, ARG0 + 2), (ARG0 + 3, ARG0 + 4)];
    b.check_all_eq_but_one(x, xs);
    b.build(&[])
}
